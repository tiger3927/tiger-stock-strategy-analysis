...
| 账户/策略查询 | 优先使用 **vnpy_mcp 工具**（实时数据），兜底使用 [redis-info.md](../redis-info.md)（缓存数据） |                     |
...
**优先方式（实时数据）：使用 vnpy_mcp 工具**

如果 AI 上下文中有可用的 `vnpy_mcp` 工具，优先使用以下 MCP 工具获取实时数据：

| 用途 | MCP 工具 | 参数 |
|------|---------|------|
| 获取账户信息 | `get_accounts` | 无参数 |
| 获取所有持仓 | `get_positions` | 无参数 |
| 获取所有策略概况 | `cta_strategies_get_all` | 无参数（含持仓、盈亏、资金占比等） |
| 获取策略详情 | `cta_strategy_get_status` | `strategy_name`（策略名称） |

> **注意**：vnpy_mcp 工具在上下文中可用仅表示 MCP 服务已注册，**不保证 vnpy 实盘系统正在运行**。调用后若返回空数据或报错，应自动降级到兜底方式（Redis 查询）。

**兜底方式（缓存数据）：使用 Redis 查询**

如果 vnpy_mcp 工具不可用，则使用 `stock_redis_query.py` 获取指定用户的量化交易账户状态和每个策略的持仓信息及运行状态：
...
从 **`cta_strategies_get_all`（优先）** 或 `stock_redis_query.py strategies <用户名>`（兜底）的输出中提取所有策略名称，得到一个策略名列表。

#### 4.2 逐策略分析流程

对列表中的 **每个策略**，按以下决策树执行：

1. **查询策略详情** — 用 **`cta_strategy_get_status`（优先）** 或 `stock_redis_query.py detail <用户名> <策略名>`（兜底）获取策略完整数据
