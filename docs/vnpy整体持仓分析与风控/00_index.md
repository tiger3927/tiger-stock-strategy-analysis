...
3. **最小干预** — 能用 `cta_strategy_set_target_pos` 调整的，不用 `cta_strategy_close` 全平
4. **数据来源必注明** — 每个结论都要注明依据（vnpy_mcp 实时数据 / 大盘分析报告 / 联网搜索），便于追溯
...
使用 **vnpy_mcp 工具**获取量化系统实时数据：

| 用途 | vnpy_mcp 工具 | 参数 |
|------|---------|------|
| 获取账户信息 | `get_accounts` | 无参数 |
| 获取所有持仓 | `get_positions` | 无参数 |
| 获取所有策略概况 | `cta_strategies_get_all` | 无参数（含持仓、盈亏、资金占比、品种信息等） |
| 获取策略详情 | `cta_strategy_get_status` | `strategy_name`（策略名称） |
| 获取成交记录 | `get_trades` | `limit`（默认 50） |

> **注意**：调用后若返回空数据或报错，说明 vnpy 实盘系统未运行，应提示用户先启动量化系统。

详细查询方式见 [vnpy_mcp.md](../vnpy_mcp.md)。
...
从 **`cta_strategies_get_all`** 的输出中提取所有策略名称，得到一个策略名列表。

#### 4.2 逐策略分析流程

对列表中的 **每个策略**，按以下决策树执行：

1. **查询策略详情** — 用 **`cta_strategy_get_status`** 获取策略完整数据
...
3. **提取交易品种** — 从策略详情（`cta_strategy_get_status` / `cta_strategies_get_all`）中获取 `品种信息` 字段，其中已包含 `ticker`（如 NVDA）、`name_cn`（如 英伟达）、`conid`、`category`、`industry` 等，可直接用于联网搜索品种行情；若 `品种信息` 为空，则从 `vt_symbol` 字段提取（如 `4391.SMART`），优先从 [scripts/vt_symbol_info.json](../../scripts/vt_symbol_info.json) 查找，未覆盖再通过 vnpy_mcp `search_vt_symbol` / `get_contract` 查询。确定真实股票名称后，再联网搜索品种行情、新闻和技术指标。
4. **获取该策略所在板块的大盘分析结论** — 从 Step 1 的结果中取对应板块的方向和风险等级
5. **查询策略对应品种的行情和技术指标** — 联网搜索该品种当前价格、技术面、新闻事件等

```mermaid
flowchart TD
    A[查询策略详情<br>cta_strategy_get_status] --> B[识别策略类型<br>加载参考文档了解参数含义]
    B --> C[提取交易品种<br>品种信息/vt_symbol 字段]
...
| 判断维度   | 数据来源        | 判断要点             |
| ------ | ----------- | ---------------- |
| 策略盈亏   | `cta_strategy_get_status` 输出 | 浮盈/浮亏比例，持仓天数     |
| 策略方向   | `cta_strategy_get_status` 输出 | 做多/做空，与大盘方向是否一致  |
...
| 风险情形 | 建议动作 | 命令 |
|---------|---------|------|
| 大盘风险等级 = 高 | 对高风险板块、高 beta 品种、已有浮盈策略优先减仓 | `cta_strategy_set_target_pos` 降低仓位 |
| 大盘风险等级 = 极高 | 优先锁定盈利，显著降低总仓位，必要时平掉高风险策略 | `cta_strategy_set_target_pos` / `cta_strategy_close` |
| 重大事件临近（FOMC/CPI/财报） | 高杠杆策略、马丁类策略先降风险暴露 | `cta_strategy_send_notice` / `cta_strategy_set_target_pos` |
...
| 板块状态 | 建议动作 | 命令 |
|---------|---------|------|
| 持仓板块走弱，且近 5 日跑输大盘 > 3% | 优先减仓该板块相关策略 | `cta_strategy_set_target_pos` |
| 持仓板块失去主线地位，且出现回撤信号 | 对浮盈策略先锁定利润，对浮亏策略评估止损 | `cta_strategy_set_target_pos` / `cta_strategy_close` |
| 新主线板块走强，账户有闲置资金 | 可对已有对应策略小幅加仓 | `cta_strategy_set_target_pos` |
| 防御板块走强、成长板块普遍转弱 | 收缩成长和高波动策略仓位 | `cta_strategy_send_notice` / `cta_strategy_set_target_pos` |
...
| **通知**   | 板块出现风险信号但策略尚无浮亏，或潜在事件临近                              | `cta_strategy_send_notice` 发送提醒 |
| **减仓**   | 板块走弱（近 5 日跑输大盘 > 3%），或大盘风险等级为高，或策略浮盈 > +5% 且板块出现回撤信号 | `cta_strategy_set_target_pos` 降低仓位 |
| **加仓**   | 板块走强（近 5 日跑赢大盘 > 3%），大盘风险等级为低，策略方向与板块一致              | `cta_strategy_set_target_pos` 增加仓位 |
| **平仓**   | 大盘风险等级为极高，或策略浮亏 > -15% 且板块持续走弱，或策略逻辑已失效              | `cta_strategy_close` 全平 |
...
3. **如果有策略需要操作**：
   - 先用 `cta_strategy_send_notice` 通知用户（汇总所有需要调整的策略及建议）
   - 等待用户确认后再执行具体命令
