...
通过 **vnpy_mcp** 工具集直连量化系统：一方面实时获取账户、持仓、策略状态、成交记录和分析报告等数据，另一方面可直接向量化系统发送操作指令（调仓、平仓、参数设置、通知、启停策略等）。

> 查询数据为**实时**（直连量化系统），非缓存数据，**无需用户名参数**，直接调用工具即可。操作指令需量化系统正在运行，实时生效。
>
> **备注**：本文档为 vnpy_mcp 工具的使用摘要，具体工具定义、参数和返回格式，以 vnpy_mcp 工具实际返回的信息及 `get_guide` 指南为准。
