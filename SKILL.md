...
description: "股票量化策略分析工具：支持为vnpy量化软件，提供多种策略的分析和审核，根据vnpy量化软件提供的数据，或者自身通过redis服务获取的账户数据，提供基本面分析，股票当前趋势分析，当前阶段和对策分析，指标和趋势预测，以及对vnpy量化软件提交的交易订单进行下单前审核；支持的策略包括：智能马丁格尔策略、智能短期趋势策略；还可 查询分析 特定用户的 redis上的 量化交易账户信息，量化交易策略执行信息"
---
